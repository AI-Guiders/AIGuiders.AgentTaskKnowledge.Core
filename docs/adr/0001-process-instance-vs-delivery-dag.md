...
| Persisted `gate` на instance рядом с P/Q в PD | Второй SSOT; рассинхрон. → D2: только `step`; P/Q derived. |
...
**Фаза C:** MCP `process_get` → step + derived pre/post (/pre_ok/post_ok); `process_advance` soft warnings / hard where AEE; `route_next` meta `processStep`.  
